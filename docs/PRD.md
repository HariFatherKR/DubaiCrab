...
### Windows: 원클릭 설치
**목표**: 설치파일 하나로 모든 것이 완료

```
OpenKlaw-Setup-1.0.0.exe 다운로드
        ↓
   더블클릭 실행
        ↓
   "설치" 버튼 클릭
...
| ID | 항목 | 상세 |
|----|------|------|
| W01 | 단일 설치파일 | `.exe` 하나로 배포 (NSIS 또는 MSI) |
| W02 | Ollama 번들 | 설치파일에 Ollama 포함 또는 자동 다운로드 |
| W03 | 모델 자동 설치 | 첫 실행 시 Qwen2.5-3B 자동 pull |
| W04 | 관리자 권한 최소화 | 가능하면 User 권한으로 설치 |
| W05 | 설치 시간 | 5분 이내 (모델 다운로드 제외) |
| W06 | 오프라인 모드 | 모델 수동 설치 옵션 제공 |

### macOS: npm 한 줄 설치
**목표**: `npm install -g openklaw` 한 줄로 완료
...
| ID | 항목 | 상세 |
|----|------|------|
| M01 | npm global 설치 | `npm install -g openklaw` 지원 |
| M02 | 자동 Ollama 설치 | Homebrew로 자동 설치 (`brew install ollama`) |
| M03 | 모델 자동 설치 | `ollama pull qwen2.5:3b` 자동 실행 |
| M04 | CLI + GUI | 터미널에서 `openklaw` 실행 시 GUI 앱 열림 |
| M05 | 의존성 최소화 | Node.js만 필요 (Rust 빌드 불필요) |
| M06 | Apple Silicon 지원 | M1/M2/M3 네이티브 지원 |
...
└───────────────────────────────┬────────────────────────────────┘
   ...
   ┌─────────────┐      ┌─────────────┐      ┌─────────────┐
   │   Windows   │      │    macOS    │      │    Linux    │
   │  .exe 설치   │      │  npm 설치   │      │  npm 설치   │
   └──────┬──────┘      └──────┬──────┘      └──────┬──────┘
          │                    │                    │
          ▼                    ▼                    ▼
   ┌─────────────┐      ┌─────────────┐      ┌─────────────┐
   │ Ollama 번들 │      │brew install │      │curl install │
   │   자동설치   │      │   ollama    │      │   ollama    │
   └──────┬──────┘      └──────┬──────┘      └──────┬──────┘
          │                    │                    │
          └────────────────────┼────────────────────┘
...
사용자로서 (Windows), 설치파일 하나만 실행하면 모든 설정이 끝나길 원한다.
→ Ollama 포함, 모델 자동 다운로드

사용자로서 (macOS), 터미널에서 한 줄 명령어로 설치하고 싶다.
→ npm install -g openklaw
```
